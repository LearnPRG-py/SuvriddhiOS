...
export const useStore = create<Store>((set, get) => ({
    ...
    markItemCompleted: (itemId) =>
        set((state) => ({ completed: { ...state.completed, [itemId]: true } })),
    ...
    saveDraftForExercise: (itemId, code) =>
        set((state) => ({ drafts: { ...state.drafts, [itemId]: code } })),
    ...
    markDrillCompleted: (drillId) =>
        set((state) => ({
            drillsCompleted: { ...state.drillsCompleted, [drillId]: true },
        })),
    ...
    getCategoryProgress: (categories: DrillCategory[], categoryId: string) => {
        ...
        const completedCount = category.drills.filter(
            (d) => drillsCompleted[d.id],
        ).length;
        return category.drills.length > 0
            ? (completedCount / category.drills.length) * 100
            : 0;
    },
    drillDrafts: {},
    saveDraftForDrill: (drillId, code) =>
        set((state) => ({
            drillDrafts: { ...state.drillDrafts, [drillId]: code },
        })),
    getDraftForDrill: (drillId) => get().drillDrafts[drillId],
    drillAnswers: {},
    saveDrillAnswer: (drillId, answer) =>
        set((state) => ({
            drillAnswers: { ...state.drillAnswers, [drillId]: answer },
        })),
